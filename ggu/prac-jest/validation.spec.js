...
test("입력한 이메일 주소중, 로컬 파트(골뱅이 기준 앞부분)에는 영문 대소문자와 숫자, 특수문자는 덧셈기호(+), 하이픈(-), 언더바(_) 3개 외에 다른 값이 존재하면 이메일 형식이 아니다.", () => {
    ...
    expect(isEmail("sp.ar/|||t,a94@gmail.com")).toEqual(false);
    expect(isEmail(";s:pa==rta94@gmail.com")).toEqual(false);
    expect(isEmail("spa()rt{}a9[]4@gmail.com")).toEqual(false);
});
...
test("입력한 이메일 주소중, 도메인(골뱅이 기준 뒷부분)에는 영문 대소문자와 숫자, 점(.), 하이픈(-) 외에 다른 값이 존재하면 이메일 형식이 아니다.", () => {
    ...
    expect(isEmail("sparta84@gma[]il.com")).toEqual(false);
    expect(isEmail("sparta84@gm{a,,;;;::}il.com")).toEqual(false);
    expect(isEmail("sparta84@gmail.cp[om===========")).toEqual(false);
});